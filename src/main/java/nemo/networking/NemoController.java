...
public class NemoController {
    ...
    private void init_window_top(final String Title, final String file) {
        Objects.requireNonNull(Title, "NULL title");
        Objects.requireNonNull(file, "NULL file");
        try {
            final String path = "/nemo/networking/";
            URL fxmlUrl = getClass().getResource(path + file);
            Objects.requireNonNull(fxmlUrl, "FXML not found!");
            FXMLLoader fxmlLoader = new FXMLLoader(fxmlUrl);
            Region root = fxmlLoader.load();

            // wymuś CSS/layout, żeby prefWidth/prefHeight miały sens
            root.applyCss();
            root.layout();

            // Pobierz rozmiar ekranu i ustaw docelowy duży rozmiar (80% ekranu lub minimum)
            Rectangle2D vb = Screen.getPrimary().getVisualBounds();
            double screenW = vb.getWidth();
            double screenH = vb.getHeight();
            double defaultW = Math.max(1000, screenW * 0.8); // większe minimum
            double defaultH = Math.max(700, screenH * 0.8);

            // Preferowane rozmiary z FXML (jeśli sensowne)
            double prefW = root.prefWidth(-1);
            double prefH = root.prefHeight(-1);
            double startW = (prefW > 0 && prefW < 16000) ? Math.min(prefW, screenW) : defaultW;
            double startH = (prefH > 0 && prefH < 16000) ? Math.min(prefH, screenH) : defaultH;

            // Fallback - jeśli system zwraca dziwne wartości
            if (!(startW > 0)) startW = defaultW;
            if (!(startH > 0)) startH = defaultH;

            // Utwórz scenę JUŻ z określonym rozmiarem (ważne!)
            ...
            // Bezpieczne miny - wymuś większe niż domyślne "0" / -1
            double minW = root.getMinWidth();
            if (minW <= 0) minW = root.minWidth(-1);
            if (minW <= 0) minW = Math.min(600, startW * 0.6);
            double minH = root.getMinHeight();
            if (minH <= 0) minH = root.minHeight(-1);
            if (minH <= 0) minH = Math.min(400, startH * 0.6);

            newStage.setMinWidth(minW);
            newStage.setMinHeight(minH);

            // Dodatkowo ustaw widoczny rozmiar stage (brak override'ów później)
            newStage.setWidth(startW);
            newStage.setHeight(startH);
            ...
            LOGGER.log(Level.SEVERE, "Failed to open " + Title + " window", e);
        }
    }
    ...
    private void top_new_action() {
        this.init_window_top("new", "new_b.fxml");
    }

    @FXML
    private void top_network_device_action() {
        this.init_window_top("Network Device", "machine.fxml");
    }

    @FXML
    private void top_network_service_action() {
        this.init_window_top("Network Service", "machine.fxml");

    }
    @FXML
    private void top_computer_action() {
        this.init_window_top("Computer", "machine.fxml");
    }

    @FXML
    private void top_vm_action() {
        this.init_window_top("VMs", "machine.fxml");
    }

    @FXML
    private void top_server_action(){
        this.init_window_top("Server", "server.fxml");
    }
